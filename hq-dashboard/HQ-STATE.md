...
### RelayFlow (dir: SpargoEmailAutomations) — section regenerated 2026-08-26 (phase 4: the canvas→engine bridge)

**What it is:** Diagram-first email/SMS automation builder whose visual canvas and real execution engine are now ONE system: published canvas workflows — branching on clicks, waiting on events, splitting on tags — compile into a strict server-executable form and run on the scheduler, alongside the linear sequence runner, approval-gated broadcasts, full deliverability rails, and the internal doorway for other first-party services' transactional sends. Built to run a six-email welcome flow for the K² newsletter and to carry SpargoDomains' renewal notices.

**Stack:** Next.js 16 (App Router) + React 19 + TypeScript strict, Tailwind v4, React Flow 12 + dagre, Zustand, Zod v4. Supabase (Postgres, 11 migrations) for send-side data AND scheduling (pg_cron calling an authenticated tick endpoint), with a zero-config local file fallback behind every store. Transactional email provider behind a signed inbound webhook with a full event ledger; mock providers plus an engine-wide dry-run mode by default; one shared rate limiter. Vercel hosting with a code-level "public surface only" deploy gate. 36 Vitest suites (304 tests).

**Git:** four build phases plus an amendments pass, a contract-reconciliation pass, and two justify-first freestyle rounds sit as one open draft PR of per-deliverable commits; base `main` last moved 2026-08-18. Phase 4 (owner-ordered) closed the last parallel-systems gap: the canvas and the engine.

**Runs today:** Yes — zero config: the builder demo, plus the whole execution loop in dry-run against local file stores. The flagship branching template (the K² Sunday Signal welcome, three nested click branches) now runs END TO END on the real scheduler in tests: click path, timeout path, tag split, goal, crash-rewind recovery that never re-sends, suppression cancel — all on an injected clock. A compiler refuses, per node and with reasons, anything the server cannot honestly execute (no SMS provider → SMS nodes refuse; no event source → only click-waits are waitable), and one button in the canvas publish dialog launches a workflow onto the engine. Operational rails unchanged and deliberately hard to reach: dry-run by default, broadcasts off by default with per-batch approval, notices held without a real postal address, heartbeat + stuck-work detection in a 15-check health command, credential-free deployed-surface verification.

**Top 3 gaps (docs vs code):**
1. Everything is built but nothing is provisioned or merged: the draft PR awaits review, and the dedicated database project, hosting environment, webhook registration, and the five-minute schedule are owner-hands steps, scripted end to end in the deploy runbook. The six-email welcome copy in the seed is still draft.
2. The Renewal Sentinel loop's two halves speak one written agreement and the doorway answers it synchronously, proven in-process and over real localhost HTTP — but a live cross-service call between the two deployed products has never happened: both deployments and the shared credential are owner-hands steps.
3. Flow conditions on tags read only what the flow itself granted an enrollment — by design there is no server-side contact store — so external signals (e.g. "read three papers") conservatively take the no-branch until a sanctioned tagging path exists. That path, plus canvas-side visibility into server-side flow runs, is the natural next build.

**Highest-leverage next move:** The owner provisioning pass from the deploy runbook, then the credential-free surface check and a cross-service dry-run soak. Everything the two products promised each other — and everything the canvas can draw within the engine's honest subset — is now waiting only on provisioning.
...
**PrincipleFirstJobber** — Tauri 2 + SvelteKit/Svelte 5 + SQLite/Drizzle desktop app for a disciplined job search. Audit mission 2026-08-24 established what it actually is: a **finished v1 nobody ever ran** — five AI engines all calibrated to say no (a fit scorer with an explicit rejection quota the UI audits against actual verdicts, a web-search approach researcher whose default answer is "no hook found, wait", a voice-matched draft pad that tracks whether its drafts get copied, a weekly premortem check that quotes your own predicted failure mode back at you, and rule-based nudges), a 7-active-target cap, 4-step onboarding, and a Sunday review ritual. Type-clean across 785 files, zero tests, zero TODO markers; smoke-verified during the audit: builds, launches, applies all 5 migrations, and gates a fresh install to onboarding. Real README and SCOPE-AUDIT.md now in the repo; git initialized (local only, per-deliverable commits). **Parked with a wake condition:** an active job search (the owner's or a handoff recipient's) — wake-up is about half a day: real bundle identity, refresh the frozen model list, first real onboarding run.
Next: sleep until a job search starts; if it's ever killed instead, the salvage list (portable prompt patterns and the Drizzle-over-Tauri SQLite wiring) is in SCOPE-AUDIT.md.

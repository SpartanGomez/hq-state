...
**Git:** branch `master`, last commit 2026-07-30 — "Verification suite, gate runner, and fixes found by running it." The install-day worktree branch is now 17 commits ahead (last 2026-08-23) across two same-day missions and awaits merge: gap audit, auth route-inventory pinning, server-side string table with untranslated-copy gates, the one-command install-day check, deploy runbook, a shell-serving fix the gates caught, a reverse-engineered SPEC.md the repo can audit itself against, and the built server-dictation fallback.
...
2. The two live-model safety evals (one-question discipline, hostile-letter red-team) remain **blocked on billing, not code** — every reachable Anthropic account on this machine is out of credit or revoked (probed 2026-08-23; unblock is ~5 minutes in the billing console, ~US$1 per eval round). Everything else automatable is green: 10/10 runnable gates, zero test failures.
...
**Highest-leverage next move:** Fund one eval round and run it green, merge the branch, deploy with the one command, then the install-day visit — its five on-phone scripts (repo GATE-STATUS.md) are all that remains unverifiable by machine.

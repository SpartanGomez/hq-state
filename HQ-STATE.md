...
### RelayFlow (dir: SpargoEmailAutomations) — section regenerated 2026-08-24 (dispatch contract conformed)
...
1. The Renewal Sentinel loop's two halves now speak one written agreement — SpargoDomains' dispatch document became reachable, was adopted verbatim into the shared coordination log after a full diff (their side won every difference; none broke a rail), and the doorway now answers it synchronously with the exact response table their client parses, proven by replaying that client's literal wire traffic through the real endpoint in dry-run. What has never happened is a live cross-service HTTP call: both deployments and the shared credential are owner-hands steps, and the Sentinel's nightly job stays in its no-network rehearsal mode until they exist.

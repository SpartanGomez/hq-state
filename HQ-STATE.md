...
### RelayFlow (dir: SpargoEmailAutomations) — section regenerated 2026-08-24 (dispatch conformed + freestyle round 2)
...
**Stack:** Next.js 16 (App Router) + React 19 + TypeScript strict, Tailwind v4, React Flow 12 + dagre, Zustand, Zod v4. Supabase (Postgres, 10 migrations) for send-side data AND scheduling (pg_cron calling an authenticated tick endpoint), with a zero-config local file fallback behind every store. Transactional email provider behind a signed inbound webhook with a full event ledger; mock providers plus an engine-wide dry-run mode by default; one shared rate limiter paced at 2 requests/second. Vercel hosting with a code-level "public surface only" deploy gate. 32 Vitest suites (284 tests).

**Git:** three mission phases, an amendments pass, a contract-reconciliation pass, and a second justify-first freestyle round (execution engine → deliverability/broadcasts/templates/hardening → ledger, soft-bounce escalation, flow simulator → bulk-send philosophy retrofit + the internal send doorway → that doorway conformed to SpargoDomains' published dispatch agreement → scheduler heartbeat, notices panel, live-HTTP contract rehearsal) sit as one open draft PR of per-deliverable commits; base `main` last moved 2026-08-18.

**Runs today:** Yes — zero config: the builder demo, plus the whole execution loop in dry-run against local file stores. One-command sequence rehearsal on an injected clock, a step-preview renderer, a 14-check health command, and a one-command dispatch rehearsal that boots the real production build with the public-surface gate ON and proves the Sentinel agreement over live localhost HTTP (10/10 in-session). The scheduler now records a heartbeat on every run — the operator UI and the health command flag a quiet scheduler before a subscriber would. Real sending stays deliberately unprovisioned AND deliberately hard to reach: broadcast mode is off by default with per-batch operator approval; internal transactional notices are held unsent until the brand row carries a real postal address (now visible in a read-only operator panel); the engine refuses double-sends by construction and auto-suppresses on hard bounce, complaint, or repeated soft bounces.

**Top 3 gaps (docs vs code):**
1. The Renewal Sentinel loop's two halves speak one written agreement (SpargoDomains' dispatch document, adopted verbatim into the shared coordination log after a full diff — their side won every difference) and the doorway now answers it synchronously, proven both in-process and over real localhost HTTP against the production build. What has never happened is a live cross-service call between the two deployed products: both deployments and the shared credential are owner-hands steps, and the Sentinel's nightly job stays in its no-network rehearsal mode until they exist.
...
**Highest-leverage next move:** The owner provisioning pass from the deploy runbook — now including handing the doorway's shared credential to the Sentinel and pointing its dispatch address at the live endpoint — then a cross-service dry-run soak before either side goes live. That turns both products' halves into the first real standing workload: compliant renewal notices, end to end.
